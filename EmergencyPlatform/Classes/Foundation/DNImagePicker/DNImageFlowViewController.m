...
- (void)setupView
{
    ...
    //将选好的数字带过去
    if (self.picCount > 0) {
        self.sendButton.badgeValue = [NSString stringWithFormat:@"%@",@(self.picCount)];
    }
...
- (BOOL)addAssetsObject:(ALAsset *)asset
{
    //限制选择图片张数
    if (self.selectedAssetsArray.count >= self.selectMaxCount) {
        showFadeOutText([NSString stringWithFormat:@"最多选择%ld张图片",self.selectMaxCount], 0, 1);
        return NO;
    }
...
- (void)sendImages
{
    ...
    for (int i = 0; i < self.selectedAssetsArray.count; ++i) {
        ALAsset *asset = self.selectedAssetsArray[i];
    ...
    if (imagePicker && [imagePicker.imagePickerDelegate respondsToSelector:@selector(dnImagePickerController:sendImages:isFullImage:imageArray:)]) {
        [imagePicker.imagePickerDelegate dnImagePickerController:imagePicker sendImages:[self seletedDNAssetArray] isFullImage:self.isFullImage imageArray:imageArray];
    }
...
- (BOOL)seletedAssets:(ALAsset *)asset
{
    ...
    if ((self.picCount + self.selectedAssetsArray.count) >= kDNImageFlowMaxSeletedNumber) {
        showFadeOutText([NSString stringWithFormat:@"最多选择%ld张图片",self.selectMaxCount], 0, 1);
        return NO;
    }
    BOOL result = [self addAssetsObject:asset];
    self.sendButton.badgeValue = [NSString stringWithFormat:@"%@",@(self.picCount + self.selectedAssetsArray.count)];
    return result;
}
...
- (UICollectionView *)imageFlowCollectionView
{
    if (nil == _imageFlowCollectionView) {
        ...
        _imageFlowCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, self.view.bounds.size.height-64) collectionViewLayout:layout];
...
{
    if (nil == _sendButton) {
        _sendButton = [[DNSendButton alloc] initWithFrame:CGRectMake(0, 0, 60, 44)];
