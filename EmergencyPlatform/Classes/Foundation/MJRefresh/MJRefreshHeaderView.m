...
- (void)adjustStateWithContentOffset
{
    ...
    if (self.scrollView.isDragging) {
        ...
        //开始刷新最多保持5 秒 add by modeguang
        if (self.hadEndSeconds == -1) {
            self.hadEndSeconds = DefalutHadEndSecond;
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                while (self.hadEndSeconds > 0) {
                    self.hadEndSeconds -= 1;
...
//                    self.scrollView.mj_contentInsetTop -= self.mj_height;
                    //解决不断刷新显示不全的bug  add   by  jianqinglian
