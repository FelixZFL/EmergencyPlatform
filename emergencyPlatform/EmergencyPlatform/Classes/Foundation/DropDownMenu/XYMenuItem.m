...
+ (instancetype)menuItem:(NSString *)title image:(UIImage *)image target:(id)target action:(SEL)action {
    XYMenuItem *item = [[XYMenuItem alloc] init:title image:image tag:0 userInfo:nil target:target action:action];
...
+ (instancetype)menuItem:(NSString *)title image:(UIImage *)image tag:(NSInteger)tag userInfo:(NSDictionary *)userInfo {
    XYMenuItem *item = [[XYMenuItem alloc] init:title image:image tag:tag userInfo:userInfo target:nil action:NULL];
    item.alignment = NSTextAlignmentCenter;
...
+ (instancetype)menuTitle:(NSString *)title WithIcon:(UIImage *)icon {
    XYMenuItem *item = [[XYMenuItem alloc] init:title image:icon tag:0 userInfo:nil target:nil action:NULL];
