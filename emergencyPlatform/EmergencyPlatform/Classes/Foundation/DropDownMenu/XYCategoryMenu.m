...
NSString * const XYCategoryMenuWillAppearNotification = @"XYCategoryMenuWillAppearNotification";
NSString * const XYCategoryMenuDidAppearNotification = @"XYCategoryMenuDidAppearNotification";
NSString * const XYCategoryMenuWillDisappearNotification = @"XYCategoryMenuWillDisappearNotification";
NSString * const XYCategoryMenuDidDisappearNotification = @"XYCategoryMenuDidDisappearNotification";
...
/// 背景色效果
static XYCategoryMenuBackgrounColorEffect   gBackgroundColorEffect = XYCategoryMenuBackgrounColorEffectSolid;
/// 是否显示阴影
...
typedef enum {
    ...
    XYCategoryMenuViewArrowDirectionNone,
    XYCategoryMenuViewArrowDirectionUp,
    XYCategoryMenuViewArrowDirectionDown,
    XYCategoryMenuViewArrowDirectionLeft,
    XYCategoryMenuViewArrowDirectionRight,
    
} XYCategoryMenuViewArrowDirection;


@interface XYCategoryMenuView : UIView
...
@interface XYCategoryMenuOverlay : UIView

@end

@interface XYCategoryMenu ()
...
#pragma mark - XYCategoryMenuOverlay

@implementation XYCategoryMenuOverlay
...
- (void)singleTap:(UITapGestureRecognizer *)recognizer {
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[XYCategoryMenuView class]] && [view respondsToSelector:@selector(dismissMenu:)]) {
...
#pragma mark - XYCategoryMenuView
@implementation XYCategoryMenuView {
    XYCategoryMenuViewArrowDirection _arrowDirection;
    CGFloat _arrowPosition;
    UIView *_contentView;
    NSArray *_menuItems;
    XYCategoryMenuSelectedItem _selectedItem;
}
...
- (id)init {
    ...
    if(self) {
        ...
        if ([XYCategoryMenu hasShadow]) {
            self.layer.shadowOpacity = 0.5;
            self.layer.shadowColor = [XYCategoryMenu tintColor].CGColor;
...
- (void)setupFrameInView:(UIView *)view fromRect:(CGRect)fromRect {
    ...
    const CGFloat widthPlusArrow = contentSize.width + [XYCategoryMenu arrowSize];
    const CGFloat heightPlusArrow = contentSize.height + [XYCategoryMenu arrowSize];
    ...
    if (heightPlusArrow < (outerHeight - rectY1)) {
        ...
        _arrowDirection = XYCategoryMenuViewArrowDirectionUp;
        ...
        _contentView.frame = (CGRect){0, [XYCategoryMenu arrowSize], contentSize};
        
        self.frame = (CGRect) {
            
            point,
            contentSize.width,
            contentSize.height + [XYCategoryMenu arrowSize]
        };
        
    } else if (heightPlusArrow < rectY0) {
        
        _arrowDirection = XYCategoryMenuViewArrowDirectionDown;
        CGPoint point = (CGPoint){
            rectXM - widthHalf,
        ...
        _arrowDirection = XYCategoryMenuViewArrowDirectionLeft;
        CGPoint point = (CGPoint){
            rectX1,
            rectYM - heightHalf
        };
        ...
        _contentView.frame = (CGRect){[XYCategoryMenu arrowSize], 0, contentSize};
        
        self.frame = (CGRect) {
            
            point,
            contentSize.width + [XYCategoryMenu arrowSize],
            contentSize.height
        };
        
    } else if (widthPlusArrow < rectX0) {
        
        _arrowDirection = XYCategoryMenuViewArrowDirectionRight;
        CGPoint point = (CGPoint){
        ...
        _arrowDirection = XYCategoryMenuViewArrowDirectionNone;
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(XYCategoryMenuSelectedItem)selectedItem {
    ...
    XYCategoryMenuOverlay *overlay = [[XYCategoryMenuOverlay alloc] initWithFrame:view.bounds];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:XYCategoryMenuWillAppearNotification object:nil];
    ...
    [UIView animateWithDuration:0.2 animations:^(void) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:XYCategoryMenuDidAppearNotification object:nil];
    }];
}

- (void)dismissMenu:(BOOL)animated {
    if (self.superview) {
        [[NSNotificationCenter defaultCenter] postNotificationName:XYCategoryMenuWillDisappearNotification object:nil];
        
        __weak typeof(self) weakSelf = self;
        void (^removeView)(void) = ^(void) {
            if ([weakSelf.superview isKindOfClass:[XYCategoryMenuOverlay class]])
                [weakSelf.superview removeFromSuperview];
            [weakSelf removeFromSuperview];
            
            [[NSNotificationCenter defaultCenter] postNotificationName:XYCategoryMenuDidDisappearNotification object:nil];
        };
    ...
    [XYCategoryMenu sharedMenu].isShow = NO;
}
...
- (UIView *)mkContentView {
    ...
    UIFont *titleFont = [XYCategoryMenu titleFont];
    ...
    for (XYMenuItem *menuItem in _menuItems) {
        ...
        const CGFloat itemHeight = MAX(titleSize.height, imageSize.height) + [XYCategoryMenu menuItemMarginY] * 2;
    ...
    UIImage *selectedImage = [XYCategoryMenuView selectedImage:(CGSize){maxItemWidth, maxItemHeight}];
    
    UIImage *gradientLine = [XYCategoryMenuView gradientLineWithColor:[XYCategoryMenu separatorColor]];
    ...
    for (XYMenuItem *menuItem in _menuItems) {
        ...
        if (menuItem.title.length) {
            ...
            if (!menuItem.enabled && !menuItem.image) {
                ...
                titleFrame = (CGRect){
                    kMarginX * 2,
                    [XYCategoryMenu menuItemMarginY],
                    maxItemWidth - kMarginX * 4,
                    maxItemHeight - [XYCategoryMenu menuItemMarginY] * 2
                };
        ...
        if (menuItem.image) {
            ...
            const CGRect imageFrame = {kMarginX * 2, [XYCategoryMenu menuItemMarginY], maxImageWidth, maxItemHeight - [XYCategoryMenu menuItemMarginY] * 2};
...
- (CGPoint)arrowPoint {
    ...
    if (_arrowDirection == XYCategoryMenuViewArrowDirectionUp) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMinY(self.frame) };
        
    } else if (_arrowDirection == XYCategoryMenuViewArrowDirectionDown) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMaxY(self.frame) };
        
    } else if (_arrowDirection == XYCategoryMenuViewArrowDirectionLeft) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame), CGRectGetMinY(self.frame) + _arrowPosition  };
        
    } else if (_arrowDirection == XYCategoryMenuViewArrowDirectionRight) {
...
+ (UIImage *)selectedImage:(CGSize)size {
    ...
    CGContextSetFillColorWithColor(context, [[XYCategoryMenu selectedColor] CGColor]);
...
- (void)drawBackground:(CGRect)frame inContext:(CGContextRef) context {
    ...
    UIColor *tintColor = [XYCategoryMenu tintColor];
    if (tintColor) {
        CGFloat a;
    ...
    if ([XYCategoryMenu backgrounColorEffect] == XYCategoryMenuBackgrounColorEffectGradient) {
        R1-=0.2;
        G1-=0.2;
        B1-=0.2;
    }
    ...
    if (_arrowDirection == XYCategoryMenuViewArrowDirectionUp) {
        ...
        const CGFloat arrowX0 = arrowXM - [XYCategoryMenu arrowSize];
        const CGFloat arrowX1 = arrowXM + [XYCategoryMenu arrowSize];
        const CGFloat arrowY0 = Y0;
        const CGFloat arrowY1 = Y0 + [XYCategoryMenu arrowSize] + kEmbedFix;
        ...
        const CGFloat arrowX0 = arrowXM - [XYCategoryMenu arrowSize];
        const CGFloat arrowX1 = arrowXM + [XYCategoryMenu arrowSize];
        const CGFloat arrowY0 = Y1 - [XYCategoryMenu arrowSize] - kEmbedFix;
        ...
        Y1 -= [XYCategoryMenu arrowSize];
        
    } else if (_arrowDirection == XYCategoryMenuViewArrowDirectionLeft) {
        
        const CGFloat arrowYM = _arrowPosition;
        const CGFloat arrowX0 = X0;
        const CGFloat arrowX1 = X0 + [XYCategoryMenu arrowSize] + kEmbedFix;
        const CGFloat arrowY0 = arrowYM - [XYCategoryMenu arrowSize];;
        const CGFloat arrowY1 = arrowYM + [XYCategoryMenu arrowSize];
        ...
        X0 += [XYCategoryMenu arrowSize];
        
    } else if (_arrowDirection == XYCategoryMenuViewArrowDirectionRight) {
        
        const CGFloat arrowYM = _arrowPosition;
        const CGFloat arrowX0 = X1;
        const CGFloat arrowX1 = X1 - [XYCategoryMenu arrowSize] - kEmbedFix;
        const CGFloat arrowY0 = arrowYM - [XYCategoryMenu arrowSize];;
        const CGFloat arrowY1 = arrowYM + [XYCategoryMenu arrowSize];
    ...
    UIBezierPath *borderPath = [UIBezierPath bezierPathWithRoundedRect:bodyFrame
                                                          cornerRadius:[XYCategoryMenu cornerRadius]];
    ...
    if (_arrowDirection == XYCategoryMenuViewArrowDirectionLeft ||
        _arrowDirection == XYCategoryMenuViewArrowDirectionRight) {
...
#pragma mark - XYCategoryMenu
/// MenuView
static XYCategoryMenu                      *gMenu;

@implementation XYCategoryMenu {
    XYCategoryMenuView *_menuView;
    BOOL         _observing;
}
...
+ (instancetype)sharedMenu {
    ...
    dispatch_once(&onceToken, ^{
        gMenu = [[XYCategoryMenu alloc] init];
    });
    return gMenu;
}
...
+ (void)reset {
    ...
    gBackgroundColorEffect = XYCategoryMenuBackgrounColorEffectSolid;
    gHasShadow = NO;
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(XYCategoryMenuSelectedItem)selectedItem {
    ...
    // 创建MenuView
    _menuView = [[XYCategoryMenuView alloc] init];
